...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    cout << "EKF: " << endl;
    ekf_.x_ = VectorXd(4);
    ekf_.x_ << 1, 1, 1, 1; //this value is important for rmse.. can be modified

    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      // TODO: Convert radar from polar to cartesian coordinates 
      //         and initialize state.
//       double rho = measurement_pack.raw_measurements_[0]; // range
//   	  double phi = measurement_pack.raw_measurements_[1]; // theta
//   	  double rho_dot = measurement_pack.raw_measurements_[2]; // ro_dot
...
//       ro_dot = measurement_pack.raw_measurements_[2];
      ...
      if ((measurement_pack.raw_measurements_[0] * cos(measurement_pack.raw_measurements_[1])) > 0.0001)
      {
...
//       }
      ...
      if ((measurement_pack.raw_measurements_[0] * sin(measurement_pack.raw_measurements_[1])) > 0.0001)
      {
...
//       ekf_.x_ << x, y, vx , vy;
   ...
   */
  FusionEKF fusionEKF;
  fusionEKF.Choosesensor_type_ = LASER_;
